...
import { MOCK_VIP_SEATS } from '../models/mock-data';
import { VipSeat } from '../models/vip-seat';
...
export class GenerateSeatsService {
    ...
    generateSeats(): any[] {
        ...
        const vipSeats = new Map<string, VipSeat>();
        MOCK_VIP_SEATS.forEach(vipSeat => {
            vipSeats.set(vipSeat.seatId, { seatId: vipSeat.seatId, price: 150 });
        });

        const regularPrice = 75;
            ...
            if (section.name === 'Right Foyer') {
                for (const rowLetter of Object.keys(rightFoyerLayout)) {
                    ...
                    for (let j = numSeats; j >= 1; j--) {
                        ...
                        const isVip = vipSeats.has(seatNumber);
                        row.push({
                            number: seatNumber,
                            section: section.name,
                            selected: false,
                            occupied: Math.random() > 0.8,
                            seatType: isVip ? 'VIP' : 'senior',
                            price: isVip ? vipSeats.get(seatNumber)!.price : regularPrice
                        });
                    }
